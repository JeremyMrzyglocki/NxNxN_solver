...
static inline bool is_space(char c) { return isspace(static_cast<unsigned char>(c)) != 0; }
static inline bool is_digit(char c) { return isdigit(static_cast<unsigned char>(c)) != 0; }
...
static inline bool is_face(char c) {return c=='R'||c=='L'||c=='F'||c=='B'||c=='U'||c=='D';}

static inline void fix_commas(string& s) {
...
static FormulaInfo tokenize_general(const string& s) {
    ...
    else if (info.pCount == 1 && info.qCount == 1) info.type = 'p'; // tie-breaker
    else info.type = 0;
    return info;
}
...
static bool parse_table_line(const string& line, int& i, int& j, int& k, string& general) {
    ...
    auto read_int = [&](int& out)->bool{
        ...
        out = stoi(line.substr(start, p - start));
        return true;
    };
...
static inline bool find_formula_with_permutations(
    const unordered_map<Triple, FormulaInfo, TripleHash>& table,
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--M")      { need(i+1<argc, "--M");      M = stoi(argv[++i]); }
        else if (a == "-h" || a == "--help") {
        cout <<
R"(Usage: rearrange --cycles <cycles_waveX.txt> --table <table_with_formula.txt> --out <wave_X_parallel.txt> --M <size>
    ...
    unordered_map<Triple, FormulaInfo, TripleHash> table;
    {
        ifstream in(table_path);
        if (!in) { cerr << "Failed to open " << table_path << "\n"; return 1;}
        string line; 
        int i,j,k; 
        string gen;
        size_t count = 0;
        while (getline(in, line)) {
            ...
            table.emplace(t, move(fi));
            ++count;
        }
        cerr << "Loaded " << count << " general formulas from " << table_path << "\n";
    }

    // 2) Prepare bucket groups for cycles (skip ambiguous)
    unordered_map<Triple, BucketGroup, TripleHash> groups;
    ...
    for (const auto& kv : table) {
        ...
        groups.emplace(t, move(g));
    }

    // 3) Read cycles file and bucket into direct/inverted
    {
        ifstream in(cycles_path);
        if (!in) { cerr << "Failed to open " << cycles_path << "\n"; return 1;}
        string line; 
        int a,b,i,j,k;
        size_t fed = 0, skipped = 0;
        while (getline(in, line)) {
            if (!parse_cycles_line(line, a, b, i, j, k)) { skipped++; continue; }
            if (a == b || a == M || b == M) {  // filter out the diag centers, edges and corners
                continue;
            }
            ...
            if (gi == groups.end()) {
                ...
                gi = groups.emplace(chosen, move(g)).first;
            }
        ...
        cerr << "Bucketed " << fed << " pairs (" << skipped << " skipped)\n";
    }

    // 4) Emit parallelized lines
    ofstream out(out_path);
    if (!out) { cerr << "Failed to create " << out_path << "\n"; return 1; }

    vector<Triple> order;
    order.reserve(groups.size());
    for (const auto& kv : groups) order.push_back(kv.first);
    sort(order.begin(), order.end(), [](const Triple& a, const Triple& b){
    ...
    for (const Triple& t : order) {
        ...
        string swapped = swap_commutator_contents(fi.general);
        FormulaInfo fi_swapped = tokenize_general(swapped);

        auto emit_bucket_map = [&](const unordered_map<int, vector<int>>& map,
